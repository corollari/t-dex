...
namespace NEP5
{
    public class NEP5 : SmartContract
    {
        ...
        public static object Main(string method, object[] args)
        {
            else if (Runtime.Trigger == TriggerType.Application)
            {
                ...
                if (method == "mint") return Mint((byte[])args[0], (BigInteger)args[2]);

                if (method == "burn") return Burn((byte[])args[0], (BigInteger)args[2]);
            }
            return false;
        }
        ...
        private static bool Mint(byte[] to, BigInteger amount)
	{
        ...
        private static bool Burn(byte[] from, BigInteger amount){
            if (from.Length != 20)
                throw new InvalidOperationException("The parameters from and to SHOULD be 20-byte addresses.");
            if (amount <= 0)
                throw new InvalidOperationException("The parameter amount MUST be greater than 0.");
